...
export const updateMaterialsFinishes = (
  roomName: string,
  props?: object,
  progress?: number
): MaterialsFinishesAction => {
  return {
      ...
      return {
        ...
        materialsFinishes: state.materialsFinishes.map((room) => {
          ...
          if (room.room.name === action.payload.roomName) {
            const PropKey = action.payload.props
              ? Object.keys(action.payload.props)[0]
              : null;
            if (
              ...
              Object.entries(room.room.props).forEach((value, key) => {
                if (PropKey) {
                  Object.entries(action.payload.props).map(
              ...
              return {
                room: {
                  name: action.payload.roomName,
                  props: updateProps ? updateProps : null,
                  progress: action.payload.progress,
                },
              };
            }
